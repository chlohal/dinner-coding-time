...
var BANNED_PATHS = ["/preview", "/contribute/*", "/-partials"];
...
var searchIndex = {
    /**
     * 
     * @param {import("./parse-controller").Page} page 
     */
    add: function add(page) {

        //don't scan partials
        if (page.location.startsWith("/-")) return false;

        //respect banned paths
        if (BANNED_PATHS.find(x => page.location.startsWith(x))) return false;
        ...
        searchDocuments.push({
            title: title,
            text: mainContent,
            address: page.location.replace(/index\.html$/, "")
        });
    },
    write: function () {

        var fuse = Fuse.createIndex([
            "title",
            "text",
            "address"
        ], searchDocuments);

        var searchFunctionAddr = path.join(__dirname, "../functions/search.js");

        var scriptSrc = fs.readFileSync(searchFunctionAddr).toString();

        var startIndex = scriptSrc.indexOf("/*START INDEX*/");
        var endIndex = scriptSrc.indexOf("/*END INDEX*/");

        var newSrc = scriptSrc.substring(0, startIndex + ("/*START INDEX*/").length) +
            `
index = ${JSON.stringify(fuse)};
list = ${JSON.stringify(searchDocuments)};
`+ scriptSrc.substring(endIndex);

        fs.writeFileSync(searchFunctionAddr, newSrc);
    },
    reset: function() {
        searchDocuments = [];
    }
 ...
 * @returns {import("./parse-controller").Page} The page
 */
